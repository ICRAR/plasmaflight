...
@dataclass
class FlightKey:
    descriptor_type: int # type(flight.DescriptorType.value)
    command: Optional[str]
    path: Optional[Tuple[bytearray]]
...
class FlightServer(flight.FlightServerBase):
    def __init__(self, host="localhost", location:str=None,
        ...
        self.flights: Dict[FlightKey, pyarrow.Table] = {} ## replace this memory store with plasma store
        self.host = host
    ...
    def list_flights(self, context, criteria) -> flight.FlightInfo:
        for key, table in self.flights.items():
            if key[1] is not None:
                descriptor = \
                    flight.FlightDescriptor.for_command(key[1])
            else:
